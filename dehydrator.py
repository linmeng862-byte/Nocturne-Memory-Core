...
分析规则：
1. domain（主题域）：必须从以下 7 个中选 1~2 个，不允许自创：
   恋爱 — 约会、告白、情话、亲密、吃醋、想念
   情绪 — 开心、难过、焦虑、感动、生气、委屈、累
   人际 — 家人、朋友、社交、同事、边界
   偏好 — 饮食、穿搭、日常、阅读、游戏、音乐、创作
   工作 — 编程、AI、硬件、项目、debug、设计
   学习 — 考试、读书、技能、成长、探索
   健康 — 身体、睡眠、运动、心理
...
class Dehydrator:
    ...
    def __init__(self, config: dict):
        ...
        self.model = dehy_cfg.get("model", "") or os.environ.get("OMBRE_MODEL", "") or "deepseek-chat"
    ...
    async def dehydrate(self, content: str, metadata: dict = None) -> str:
        ...
        # --- Always show original content, no API compression ---
        # --- 始终显示原文，不走API压缩 ---
    ...
    def _extract_readable_content(content: str) -> str:
        """
        If content is a JSON object (from dehydration API), extract only core_facts
        as readable text. Other fields (emotion_state, todos, keywords, summary) are
        discarded — they're internal compression artifacts, not memory narratives.
        如果content是JSON（脱水API输出），只取core_facts展成自然语言，其余字段丢弃。
        """
        ...
        try:
            ...
            if not isinstance(obj, dict):
                return content
            facts = obj.get("core_facts", [])
            if facts and isinstance(facts, list):
                return "，".join(str(f) for f in facts if f)
            # fallback: summary if no core_facts
    ...
    def _format_output(self, content: str, metadata: dict = None) -> str:
        ...
        将脱水结果格式化为可注入上下文的文本。
        """
        content = self._extract_readable_content(content)
